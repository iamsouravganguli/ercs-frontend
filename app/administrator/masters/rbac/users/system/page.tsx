  ...
  useEffect(() => {
    const handleMessage = (event: MessageEvent) => {
      if (event.data === "REFRESH_USER_LIST") {
        data.refetch();
      } else if (event.data?.type === "APPLY_SYSTEM_USER_FILTERS") {
        setQuery({
          ...query,
          ...event.data.filters,
          page: 1,
        });
      } else if (event.data?.type === "RESET_SYSTEM_USER_FILTERS") {
        setQuery({
          ...query,
          phone: undefined,
          email: undefined,
          ...
          <div className="flex items-center gap-1">
            <Button
              ...
              onClick={(e) => {
                e.stopPropagation();
                openCenteredPopup(
                  `/action/users/system/view?username=${item.username}`,
                  "View System User",
                );
              }}
            >
            ...
            <Button
              ...
              onClick={(e) => {
                e.stopPropagation();
                openCenteredPopup(
                  `/action/users/system/edit?username=${item.username}`,
                  "Edit System User",
                );
              }}
            >
      ...
      <div className="sticky top-0 z-20 bg-[#dbeafe] dark:bg-slate-900 border-b border-blue-200 dark:border-blue-900 w-full flex flex-col md:flex-row md:items-center justify-between gap-2.5 md:gap-3 px-4 py-2.5 md:py-0 h-auto md:h-14">
        ...
        <div className="flex items-center gap-3 w-full md:w-auto">
          ...
          <span className="font-bold text-base sm:text-lg text-foreground tracking-tight shrink-0">
            {t("page_tab.system_user") || "System Users"}
          </span>
        </div>
        ...
        <div className="flex items-center gap-2 w-full md:w-auto">
          ...
          <Button
            ...
            onClick={() => {
              ...
              openCenteredPopup(
                `/action/users/system/filter?${params.toString()}`,
                "Filter System Users",
                520,
                700,
              );
