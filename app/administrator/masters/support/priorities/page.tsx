...
export default function SupportPrioritiesPage() {
  ...
  const data = useSupportMasterList("support-priorities", {
    ...query,
  });
  ...
  const handleOpenAdd = () => {
    openCenteredPopup("/action/support/priorities/add", "Add Priority");
  };

  const handleOpenEdit = (id: string) => {
    openCenteredPopup(
      `/action/support/priorities/edit?id=${id}`,
      "Edit Priority",
    );
  };
  ...
  const handleOpenView = (id: string) => {
    openCenteredPopup(
      `/action/support/priorities/view?id=${id}`,
      "View Priority",
    );
  };

  const columns: ColumnDef<PriorityData>[] = [
    {
      accessorKey: "code",
    ...
    {
      accessorKey: "name_en",
      header: t("table.name") || "Priority Level",
      cell: ({ row }) => {
        return (
    ...
    },
    {
      ...
      cell: ({ row }) => {
        const unit =
          row.original.response_time_unit === "minutes" ? "min" : "hrs";
        return (
          <span className="font-medium text-slate-700 dark:text-slate-300">
            {row.original.response_time_hours} {unit}
          </span>
        );
      },
    },
    {
      ...
      cell: ({ row }) => {
        const unit =
          row.original.resolution_time_unit === "minutes" ? "min" : "hrs";
        return (
          <span className="font-semibold text-red-600 dark:text-red-400">
            {row.original.resolution_time_hours} {unit}
          </span>
        );
      },
    },
  ...
  return (
    <div className="w-full h-full flex flex-col bg-background overflow-hidden">
      <div className="sticky top-0 z-20 bg-[#dbeafe] dark:bg-slate-900 border-b border-blue-200 dark:border-blue-900 w-full flex flex-col md:flex-row md:items-center justify-between gap-2.5 md:gap-3 px-4 py-2.5 md:py-0 h-auto md:h-14">
        <div className="flex items-center gap-3 w-full md:w-auto">
          ...
          <span className="font-bold text-base sm:text-lg text-foreground tracking-tight shrink-0">
            {t("page_tab.support_priorities") || "Support Priorities"}
          </span>
        </div>
      ...
      <div className="w-full flex-1 flex flex-col min-h-0 overflow-hidden">
        <DataTable
          data={(data?.data?.result?.data ?? []) as PriorityData[]}
          columns={columns}
