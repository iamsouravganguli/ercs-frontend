...
export default function SupportTypesPage() {
  ...
  const data = useSupportMasterList("support-types", {
    ...query,
  });
  ...
  const handleOpenAdd = () => {
    openCenteredPopup("/action/support/types/add", "Add Support Type");
  };

  const handleOpenEdit = (id: string) => {
    openCenteredPopup(
      `/action/support/types/edit?id=${id}`,
      "Edit Support Type",
    );
  };
  ...
  const handleOpenView = (id: string) => {
    openCenteredPopup(
      `/action/support/types/view?id=${id}`,
      "View Support Type",
    );
  };

  const columns: ColumnDef<SupportTypeData>[] = [
    {
      accessorKey: "code",
    ...
    {
      accessorKey: "name_en",
      header: t("table.name") || "Support Type Name",
      cell: ({ row }) => {
        return (
...

    {
      ...
      header: t("table.display_order") || "Display Order",
      cell: ({ row }) => (
        <span className="text-xs font-mono">{row.original.display_order}</span>
      ),
    },
    {
  ...
  return (
    <div className="w-full h-full flex flex-col bg-background overflow-hidden">
      <div className="sticky top-0 z-20 bg-[#dbeafe] dark:bg-slate-900 border-b border-blue-200 dark:border-blue-900 w-full flex flex-col md:flex-row md:items-center justify-between gap-2.5 md:gap-3 px-4 py-2.5 md:py-0 h-auto md:h-14">
        <div className="flex items-center gap-3 w-full md:w-auto">
          ...
          <span className="font-bold text-base sm:text-lg text-foreground tracking-tight shrink-0">
            {t("page_tab.support_types") || "Support Types"}
          </span>
        </div>
      ...
      <div className="w-full flex-1 flex flex-col min-h-0 overflow-hidden">
        <DataTable
          data={(data?.data?.result?.data ?? []) as SupportTypeData[]}
          columns={columns}
