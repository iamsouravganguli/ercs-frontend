...
export default function SupportResolutionTypesPage() {
  ...
  const data = useSupportMasterList("support-resolution-types", {
    ...query,
  });
  ...
  const handleOpenAdd = () => {
    openCenteredPopup(
      "/action/support/resolution-types/add",
      "Add Resolution Type",
    );
  };
  ...
  const handleOpenEdit = (id: string) => {
    openCenteredPopup(
      `/action/support/resolution-types/edit?id=${id}`,
      "Edit Resolution Type",
    );
  };

  const handleOpenView = (id: string) => {
    openCenteredPopup(
      `/action/support/resolution-types/view?id=${id}`,
      "View Resolution Type",
    );
  };
  ...
  const columns: ColumnDef<ResolutionData>[] = [
    ...
    {
      accessorKey: "name_en",
      header: t("table.name") || "Resolution Method",
      cell: ({ row }) => {
        return (
    ...
    },
    {
      ...
      cell: ({ row }) => {
        return (
          <span className="text-slate-500 dark:text-slate-400 text-xs line-clamp-1 max-w-60">
  ...
  return (
    <div className="w-full h-full flex flex-col bg-background overflow-hidden">
      <div className="sticky top-0 z-20 bg-[#dbeafe] dark:bg-slate-900 border-b border-blue-200 dark:border-blue-900 w-full flex flex-col md:flex-row md:items-center justify-between gap-2.5 md:gap-3 px-4 py-2.5 md:py-0 h-auto md:h-14">
        <div className="flex items-center gap-3 w-full md:w-auto">
          ...
          <span className="font-bold text-base sm:text-lg text-foreground tracking-tight shrink-0">
            {t("page_tab.support_resolution_types") ||
        ...
        <div className="flex items-center gap-2 w-full md:w-auto">
          <div className="flex-1 md:flex-initial">
            <SearchInput
              ...
              value={query.search ?? ""}
              placeholder="Search resolution type..."
      ...
      <div className="w-full flex-1 flex flex-col min-h-0 overflow-hidden">
        <DataTable
          data={(data?.data?.result?.data ?? []) as ResolutionData[]}
          columns={columns}
