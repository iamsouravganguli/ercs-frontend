...
export default function SupportStatusesPage() {
  ...
  const data = useSupportMasterList("support-statuses", {
    ...query,
  });
  ...
  const handleOpenAdd = () => {
    openCenteredPopup("/action/support/statuses/add", "Add Status");
  };

  const handleOpenEdit = (id: string) => {
    openCenteredPopup(`/action/support/statuses/edit?id=${id}`, "Edit Status");
  };

  const handleOpenView = (id: string) => {
    openCenteredPopup(`/action/support/statuses/view?id=${id}`, "View Status");
  };
  ...
  const columns: ColumnDef<StatusData>[] = [
    ...
    {
      accessorKey: "name_en",
      header: t("table.name") || "Status Label",
      cell: ({ row }) => {
        return (
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="flex items-center gap-2">
            ...
            />
            <code className="text-xs font-mono text-slate-500 uppercase">
              {color}
            </code>
          </div>
        );
    ...
    },
    {
      ...
      header: "Type",
      cell: ({ row }) => {
        if (row.original.is_initial) {
          return <StatusBadge variant="info">Initial State</StatusBadge>;
        }
        if (row.original.is_terminal) {
          return <StatusBadge variant="success">Terminal State</StatusBadge>;
        }
  ...
  return (
    <div className="w-full h-full flex flex-col bg-background overflow-hidden">
      <div className="sticky top-0 z-20 bg-[#dbeafe] dark:bg-slate-900 border-b border-blue-200 dark:border-blue-900 w-full flex flex-col md:flex-row md:items-center justify-between gap-2.5 md:gap-3 px-4 py-2.5 md:py-0 h-auto md:h-14">
        <div className="flex items-center gap-3 w-full md:w-auto">
          ...
          <span className="font-bold text-base sm:text-lg text-foreground tracking-tight shrink-0">
            {t("page_tab.support_statuses") || "Support Statuses"}
          </span>
        </div>
      ...
      <div className="w-full flex-1 flex flex-col min-h-0 overflow-hidden">
        <DataTable
          data={(data?.data?.result?.data ?? []) as StatusData[]}
          columns={columns}
