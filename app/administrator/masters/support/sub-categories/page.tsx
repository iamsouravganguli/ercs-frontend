...
export default function SupportSubCategoriesPage() {
  ...
  const data = useSupportMasterList("support-sub-categories", {
    ...query,
  });
  ...
  const handleOpenAdd = () => {
    openCenteredPopup("/action/support/sub-categories/add", "Add Sub-Category");
  };

  const handleOpenEdit = (id: string) => {
    openCenteredPopup(
      `/action/support/sub-categories/edit?id=${id}`,
      "Edit Sub-Category",
    );
  };
  ...
  const handleOpenView = (id: string) => {
    openCenteredPopup(
      `/action/support/sub-categories/view?id=${id}`,
      "View Sub-Category",
    );
  };

  const columns: ColumnDef<SubCategoryData>[] = [
    {
      accessorKey: "code",
    ...
    {
      accessorKey: "name_en",
      header: t("table.name") || "Sub-Category Name",
      cell: ({ row }) => {
        return (
    ...
    },
    {
      ...
      cell: ({ row }) => {
        const parent = row.original.category_detail;
        return (
          <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold bg-blue-50 text-blue-700 dark:bg-blue-950/40 dark:text-blue-300 border border-blue-100 dark:border-blue-900/50">
            {parent ? getLabel(parent, lang) : "General"}
          </span>
        );
      },
    },
  ...
  return (
    <div className="w-full h-full flex flex-col bg-background overflow-hidden">
      <div className="sticky top-0 z-20 bg-[#dbeafe] dark:bg-slate-900 border-b border-blue-200 dark:border-blue-900 w-full flex flex-col md:flex-row md:items-center justify-between gap-2.5 md:gap-3 px-4 py-2.5 md:py-0 h-auto md:h-14">
        <div className="flex items-center gap-3 w-full md:w-auto">
          ...
          <span className="font-bold text-base sm:text-lg text-foreground tracking-tight shrink-0">
            {t("page_tab.support_sub_categories") || "Support Sub-Categories"}
          </span>
        </div>
      ...
      <div className="w-full flex-1 flex flex-col min-h-0 overflow-hidden">
        <DataTable
          data={(data?.data?.result?.data ?? []) as SubCategoryData[]}
          columns={columns}
