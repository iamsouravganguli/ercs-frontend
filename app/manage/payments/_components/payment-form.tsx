...
import { useCreatePaymentOrder, usePaymentModeList, usePaymentOrderDetail, useUpdatePaymentOrder, useCaseList, useCaseDetail } from '@/lib/query';

const PaymentFormSchema = z.object({
  case_number: z.string().min(1, "Case number required"),
...
export function PaymentForm({
  ...
  const watchCaseNumber = form.watch("case_number");
  const isOffline =
  ...
  const caseListQuery = useCaseList(
    caseSearch.trim().length >= 1
      ? { search: caseSearch, limit: 20, page: 1 }
      : { limit: 20, page: 1 },
  );
  const caseOptions = React.useMemo(() => {
    const data = (caseListQuery.data as any)?.result?.data || [];
    const opts: { label: string; value: string }[] = data.map((c: any) => ({
      label: c.case_number,
      value: c.case_number,
    }));
    if (watchCaseNumber && !opts.find((o) => o.value === watchCaseNumber)) {
      opts.unshift({ label: watchCaseNumber, value: watchCaseNumber });
    }
    return opts;
  }, [caseListQuery.data, watchCaseNumber]);


  const caseDetailForMode = useCaseDetail(watchCaseNumber as string);
  useEffect(() => {
    if (isEditing || isView) return;
    if (!watchCaseNumber) return;
    const detail = (caseDetailForMode.data as any)?.result?.data;
    if (!detail) return;
    const roleCode = String(
      detail.created_by_detail?.role_code || "",
    ).toUpperCase();
    const isCitizenOrAdvocate = ["CT", "CT", "AD", "AD"].includes(roleCode);
    if (isCitizenOrAdvocate) {
      form.setValue("payment_mode", "ONLINE");
    } else {
    ...
    watchCaseNumber,
    (caseDetailForMode.data as any)?.result?.data,
    isEditing,
    isView,
  ]);
  ...
  const steps = [
    {
      ...
      fields: ["case_number", "description"] as const,
    },
    {
      id: 2,
      label: "Amount & Mode",
    ...
    { id: 3, label: "Remarks", fields: ["notes"] as const },
  ];
  ...
  useEffect(() => {
    if (apiPaymentDetail && (isEditing || isView) && paymentId) {
      ...
      form.reset({
        case_number: apiPaymentDetail.object_id || "",
  ...
  const onSubmit = form.handleSubmit(async (data) => {
    if (!isView && currentStep !== 3 && !saveTriggeredRef.current) return;
    ...
    try {
      if (isEditing && paymentId) {
        ...
        const payload: any = {
          model: "casemodel",
          object_id: data.case_number.trim(),
          amount: amountPaise,
          description: data.description.trim(),
          metadata: {
  ...
  if (isView) {
    ...
    return (
      <div className="flex flex-col h-full bg-card overflow-hidden">
        <div className="sticky top-0 z-20 flex items-center justify-between h-14 px-6 border-b bg-card shrink-0">
          <h1 className="text-lg font-semibold tracking-tight">View Payment</h1>
        </div>
                    ...
                    <div className="p-6 space-y-4">
                      <div className="grid gap-4">
                        <CustomComboboxField
                          ...
                          required
                          readOnly={isEditing || isView}
                          loading={caseListQuery.isFetching}
                          options={caseOptions}
                          onSearch={(q) => setCaseSearch(q)}
                          debounceMs={300}
                        />
                        <TextFieldV2
                          control={form.control as any}
