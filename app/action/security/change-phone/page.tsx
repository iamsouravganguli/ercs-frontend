...
import { ChangePhoneService } from "@/app/action/security/change-phone/services";
import { applyBackendErrors, useProfileDetail } from "@/lib";
import { ChangePhoneResponse } from "@/app/action/security/change-phone/types";
import { OTPModal, OTPModalHandle } from "@/common/components/otp_modal";
import toast from "react-hot-toast";
import { PhoneField } from "@/components/ui/phone-field";
import { ReadonlyPhoneField } from "@/components/ui/readonly-phone-field";
import { Save } from "lucide-react";

export default function ActionChangePhonePage() {
  ...
  const ChangePhoneMutation = useMutation({
    mutationKey: ["CHANGE_PHONE"],
    mutationFn: ChangePhoneService,
    onSuccess: (res) => {
    ...
    onError: (err: ChangePhoneResponse) => {
      applyBackendErrors(form, err.errors, err.message);
      setOtpOpen(false);
    },
  });

  const onSubmit = (_data: z.input<typeof ChangePhoneSchema>) => {
  ...
  const handleOtpVerified = () => {
    ...
    ChangePhoneMutation.mutate({ ...form.getValues() });
  };
          ...
          <div className="flex-1 overflow-y-auto px-6 py-6 space-y-6 no-scrollbar bg-muted/40">
            <div className="flex items-center justify-between">
              <div>
                <h1 className="text-lg font-bold tracking-tight">
                  {t("change_phone.title") || "Change Mobile Number"}
                </h1>
                <p className="text-xs text-muted-foreground mt-1">
                  {t("change_phone.subtitle") ||
            ...
            <section className="space-y-4 bg-card border rounded-xl p-6 shadow-sm">
              <div className="text-base font-semibold pb-2 border-b">
                {t("sections.basic_details") || "Phone Configuration"}
              </div>
              <div className="space-y-4">
                <ReadonlyPhoneField
                  label={t("phone_old.label") || "Current Mobile Number"}
                  value={Profile.data?.result?.data.phone}
                />
                <PhoneField
                  control={form.control}
                  name="identifier"
                  label={t("phone_new.label") || "New Mobile Number"}
                  placeholder={
                    t("phone_new.placeholder") || "Enter new mobile number"
                  }
                />
              </div>
            </section>
          ...
          <div className="flex items-center justify-end border-t bg-background px-8 py-4 z-10 relative shrink-0">
            <div className="flex gap-3">
              ...
              <Button
                ...
                className="px-6"
                disabled={
                  ChangePhoneMutation.isPending || !form.formState.isValid
                }
              >
